...
class RewardBatchServiceImplTest {
  ...
  void processSingleBatch_Success_NoSuspended() {
    ...
    doReturn(Mono.empty()).when(rewardBatchServiceSpy)
        .updateAndSaveRewardTransactionsToApprove(anyString(), anyString());
    when(rewardBatchRepository.save(any(RewardBatch.class))).thenAnswer(
    ...
    verify(rewardBatchServiceSpy, times(1)).updateAndSaveRewardTransactionsToApprove(
        REWARD_BATCH_ID_1, INITIATIVE_ID);
  ...
  void processSingleBatch_Success_WithSuspended() {
    ...
    doReturn(Mono.just(REWARD_BATCH_2)).when(rewardBatchServiceSpy)
        .createRewardBatchAndSave(any(RewardBatch.class));
    doReturn(Mono.empty()).when(rewardBatchServiceSpy)
        .updateAndSaveRewardTransactionsSuspended(anyString(), anyString(), anyString());
    when(rewardBatchRepository.save(any(RewardBatch.class))).thenAnswer(
        invocation -> Mono.just(invocation.getArgument(0)));
    ...
    verify(rewardBatchRepository, times(1)).save(argThat(batch ->
        batch.getId().equals(REWARD_BATCH_ID_1) && batch.getStatus()
            .equals(RewardBatchStatus.APPROVED)));
  }
  ...
  void suspendTransactions_ok() {
    ...
    when(rewardTransactionRepository.updateStatusAndReturnOld(batchId, "TX1",
        RewardBatchTrxStatus.SUSPENDED, request.getReason()))
  ...
  void suspendTransactions_noModifiedTransactions() {
    ...
    when(rewardTransactionRepository.updateStatusAndReturnOld(eq(batchId), anyString(),
        eq(RewardBatchTrxStatus.SUSPENDED), eq(request.getReason())))
  ...
  void approvedTransactions() {
    ...
    when(rewardTransactionRepository.updateStatusAndReturnOld(batchId, "trxApproved",
        RewardBatchTrxStatus.APPROVED, null))
  ...
  void rejectTransactions() {
    ...
    RewardTransaction trxApprovedMock = RewardTransaction.builder()
        ...
        .rewards(rewardApprovedMap).build();
    when(rewardTransactionRepository.updateStatusAndReturnOld(batchId, "trxApproved",
        RewardBatchTrxStatus.REJECTED, transactionsRequest.getReason()))
        .thenReturn(Mono.just(trxApprovedMock));
  ...
  void validateRewardBatch_L1ToL2_Success() {
    ...
    StepVerifier.create(
            rewardBatchService.validateRewardBatch("operator1", INITIATIVE_ID, REWARD_BATCH_ID_1))
        .verifyComplete();
  ...
  void validateRewardBatch_L2ToL3_Success() {
    ...
    StepVerifier.create(
            rewardBatchService.validateRewardBatch("operator2", INITIATIVE_ID, REWARD_BATCH_ID_1))
        .verifyComplete();
  ...
  void validateRewardBatch_RoleNotAllowed_L1() {
    ...
    StepVerifier.create(
            rewardBatchService.validateRewardBatch("wrongRole", INITIATIVE_ID, REWARD_BATCH_ID_1))
        .expectErrorSatisfies(ex -> {
  ...
  void validateRewardBatch_InvalidState() {
    ...
    StepVerifier.create(
            rewardBatchService.validateRewardBatch("operator3", INITIATIVE_ID, REWARD_BATCH_ID_1))
        .expectErrorSatisfies(ex -> {
          assertInstanceOf(ResponseStatusException.class, ex);
          assertEquals(HttpStatus.BAD_REQUEST, ((ResponseStatusException) ex).getStatusCode());
