...
@TestPropertySource(
        properties = {
                ...
                "app.invitalia.credentials.tenant-id=TENANT_KO_1",
...
class InvitaliaTokenRestClientImplErrorTest extends BaseWireMockTest {
    @Autowired
    ...
    void getToken() {
        ...
        try {
            restClient.getToken().block();
        } catch (InvitaliaConnectingErrorException e) {
            Assertions.assertEquals("Failed to retrieve Invitalia token after %d attempts".formatted(11), e.getMessage());
        }
    }
