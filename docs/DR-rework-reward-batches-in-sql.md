...
The SQL rework in [implementation-plan-rework-reward-batches-in-sql.md](implementation-plan-rework-reward-batches-in-sql.md) is complete and must not be rewritten. Reversal detach now follows [implementation-plan-refunded-batch-detach.md](implementation-plan-refunded-batch-detach.md): payment CDC publishes `TRANSACTION_REFUNDED` on `idpay-transaction`, and `rewardTrxConsumer` persists `REFUNDED` and removes the current batch association.
...
- Keep the existing external REST and Kafka contracts unless a separately approved contract change replaces them. Invoice replacement uses a dedicated, versioned `INVOICE_REPLACED` impact event carrying an event ID, outcome timestamp, canonical post-operation transaction projection, and shared monotonic transaction revision. A reversal is the existing `TRANSACTION_REFUNDED` outbox event, published by payment CDC onto `idpay-transaction`; `rewardTrxConsumer` persists `REFUNDED` and detaches current batch membership. Payment also exposes no write coupling here: it calls a narrow read-only eligibility query before committing its command.
...
| --- | --- |
| Kafka transaction synchronization | `rewardTrxConsumer` consumes versioned `RewardTransactionDTO` snapshots from `idpay-transaction` and conditionally upserts the local projection. A `TRANSACTION_REFUNDED` snapshot (payload status `REFUNDED`) must not be skipped: persist the projection and detach any current reward-batch membership. Payment CDC does not set an `operationType` header; a header must not rewrite status. An event may not change an existing transaction's initiative; the conflict is rejected/quarantined. An `INVOICED` snapshot is a local projection update only: it must not cancel or delete the payment transaction, whose ownership remains with `idpay-payment`. |
...
| Payment-driven invoice update and invoiced reversal | Payment owns the invoice update/reversal command, scope policy, blob operations, and authoritative transaction state. Before committing, payment calls this service's narrow read-only eligibility query. After an invoice replacement commits, payment publishes a dedicated `INVOICE_REPLACED` impact keyed by transaction ID with an event ID, schema version, shared transaction revision, outcome timestamp, and canonical post-operation projection. This service compares that revision with the transaction-local latest-applied-impact revision: a greater revision applies once, while an equal or lower revision is ignored. A `CREATED` source membership is kept; otherwise the current membership moves to the outcome-month grouping (creating it when absent) as `SUSPENDED`. After a reversal commits, payment writes `TRANSACTION_REFUNDED` to its outbox and CDC publishes it to `idpay-transaction`. This service persists `REFUNDED` and detaches the current membership. Neither path updates batch counters or performs payment/blob work. |
...
5. Kafka processing must be at-least-once safe: every payment snapshot, including `TRANSACTION_REFUNDED`, carries a shared monotonic transaction revision, and every `INVOICE_REPLACED` impact carries a unique event ID. Conditional projection updates, idempotent detach of an already unassigned row, and the transaction-local latest-applied-impact revision ensure retries or stale generic snapshots cannot duplicate or undo local batch membership.
...
- This decision does not move batch ownership to another service. Payment remains the owner of the payment transaction and of invoice update/reversal commands. It publishes `TRANSACTION_REFUNDED` on `idpay-transaction` for reversal detach and the dedicated `INVOICE_REPLACED` impact for invoice replacement; this service owns only the local batch effect.
