...
| --- | --- |
| Kafka transaction synchronization | `rewardTrxConsumer` consumes versioned `RewardTransactionDTO` snapshots from `idpay-transaction` and conditionally upserts the local projection. A `TRANSACTION_REFUNDED` snapshot (status `REFUNDED`, including the legacy `operationType=REFUNDED` header) must not be skipped: persist the projection and detach any current reward-batch membership. An event may not change an existing transaction's initiative; the conflict is rejected/quarantined. An `INVOICED` snapshot is a local projection update only: it must not cancel or delete the payment transaction, whose ownership remains with `idpay-payment`. |
