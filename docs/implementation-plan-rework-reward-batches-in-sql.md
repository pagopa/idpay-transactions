...
- For every numbered PR, create a new dedicated git branch before making its changes. Branch it from the branch containing that PR's declared dependencies (`migration-to-sql-database` for PR 01; the preceding dependency branch for later PRs). Commit the completed PR changes on that branch before reporting the PR as implemented. Do not use git branch names with a folder separator, such as `feat/example`.
